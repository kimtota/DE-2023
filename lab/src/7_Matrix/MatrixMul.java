...
public class MatrixMul
{
	public static class MatrixMulMapper1 extends Mapper<Object, Text, Text, IntWritable> {
		...
		private int m_value; // 2 i
		private int k_value; // 2 변수
		private int n_value; // 2 j
		...
		public void map(Object key, Text value, Context context) throws IOException, InterruptedException {
			...
			int row_id = Integer.parseInt( itr.nextToken().trim() ); // 행 위치값
			int col_id = Integer.parseInt( itr.nextToken().trim() ); // 열 위치값
			int matrix_value = Integer.parseInt( itr.nextToken().trim() ); // 데이터
